...
# Making sure row names in colData match and are
# in the same order as counts_data
...
levels(dds$condition) # Default is ordered alphabetically
print("Changing the level order...")
...
dds <- dds[keep,] # Dimension reduced here
...
# merge MAP logfoldchange values to MLE table,
# rename the logfoldchange columns to .MLE and .MAP
...
EnhancedVolcano(plot_lfc, lab = plot_lfc$hgnc_symbol, x = 'log2FoldChange', y = 'padj', ylab = expression(-log[10]~padj), legendPosition = 'none', drawConnectors = TRUE, max.overlaps = 7, title = paste(toupper(title[2]), 'vs', toupper(title[4])), pCutoff = as.numeric(Vp_Cut), FCcutoff = as.numeric(Vfc_Cut))
dev.off()
